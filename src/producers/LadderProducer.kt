...
class LadderProducer {
    ...
    fun produce(ladder : String?): Ladder {
        val json = URL(Network.ladderUrl + ladder).readText()
        val result = Network.gson.fromJson(json, Ladder::class.java)
        return result
    }

    fun produce(ladder: String?,offset:Int?): Ladder {
        val json = URL(Network.ladderUrl + ladder + "?offset=$offset").readText()
        val result = Network.gson.fromJson(json, Ladder::class.java)
        return result
    }

    fun produce(ladder: String?,offset: Int?,limit: Int?) : Ladder{
        val json = URL(Network.ladderUrl + ladder + "?offset=$offset&limit=$limit" ).readText()
        val result = Network.gson.fromJson(json, Ladder::class.java)
        return result
    }
